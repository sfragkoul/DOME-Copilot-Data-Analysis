...
Replicated from: Download_1012_Positive_PMC_Full_Text_and_Supplementary.py
Downloads PDF and Supplementary files for a negative set of entries (1012).
Includes robustness checks, automated remediation, and visualization.

Usage:
    python Download_1012_Negative_PMC_Full_Text_and_Supplementary.py [options]

Options:
    --automated         Run non-interactively.
    --force-download    Force re-download of existing files.
    --input-file        Path to input CSV (default: negative_entries_pmid_pmcid_filtered.csv)
"""
...
PDF_OUTPUT_FOLDER = 'Negative_PMC_PDFs'
SUPP_OUTPUT_FOLDER = 'Negative_PMC_Supplementary'
TSV_OUTPUT_FOLDER = 'Negative_PMC_TSV_Files'
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Download Negative PMC Full Text and Supplementary")
    ...
    parser.add_argument("--input-file", type=str, default=None, help="Input CSV file path") 
...
def find_input_file(user_path):
    candidates = [
        ...
        "../negative_entries_pmid_pmcid_filtered.csv",
        os.path.join(BASE_DIR, "negative_entries_pmid_pmcid_filtered.csv")
    ]
    for c in candidates:
...
def download_pdfs(df, output_folder, force=False):
    ...
    """
    if 'mapped_pmcid' not in df.columns or df['mapped_pmcid'].isnull().all():
        print("No Valid PMCIDs to download PDFs for.")
        return
...
def download_supplementary_files(df, output_folder, force=False):
    ...
    """
    if 'mapped_pmcid' not in df.columns or df['mapped_pmcid'].isnull().all():
        print("No Valid PMCIDs to download Supplementary files for.")
        return
...
def analyze_and_visualize(df, tsv_file):
    ...
    try:
        ...
        plot_path = os.path.join(TSV_OUTPUT_FOLDER, f'Negative_Entries_Analysis_{current_date}.png')
        ...
        bars = plt.bar(stages, vals, color=['gray', 'red', 'purple', 'yellow']) # Changed colors for negative
        plt.title('Retrieval Pipeline (Negative Dataset)')
        ...
        plt.subplot(1, 2, 2)
        if with_pmcid > 0:
            plt.pie([pdfs, with_pmcid-pdfs], labels=['PDF Available', 'Missing'], autopct='%1.1f%%', colors=['#ff9999','#e6e6e6'])
        else:
            plt.text(0.5, 0.5, "No PMCIDs", ha='center')
...
def main():
    ...
    if not infile:
        print("Error: Input file 'negative_entries_pmid_pmcid_filtered.csv' not found.")
        sys.exit(1)
        
    print(f"Loading {infile}...")
    # NOTE: Changed from read_csv(..., sep='\t') to default csv read
    ...
    # Mock other cols to avoid errors if referenced
    ...
    status_file = os.path.join(TSV_OUTPUT_FOLDER, 'negative_entries_status.tsv')
    ...
    print("\nDone! Check Negative_PMC_TSV_Files for reports.")
